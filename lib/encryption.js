...
const MAGIC = Buffer.from('HFA1'); // HF AI v1 encrypted file marker
...
class EncryptionVault {
  ...
  async initializeKey(secretStorage) {
    ...
    if (this._keySource === 'env') {
      const envKey = process.env.HFAICODE_ENCRYPTION_KEY;
      if (envKey) {
        this._masterKey = Buffer.from(envKey, 'hex');
        if (this._masterKey.length !== KEY_LENGTH) {
          throw new Error('HFAICODE_ENCRYPTION_KEY must be 64 hex characters (32 bytes)');
        }
        return;
      }
    }
    ...
    if (this._keySource === 'secretStorage' && secretStorage) {
      let stored = await secretStorage.get('hfaicode.encryptionKey');
      if (!stored) {
        // Generate and store a new key
        const newKey = crypto.randomBytes(KEY_LENGTH);
        stored = newKey.toString('hex');
        await secretStorage.store('hfaicode.encryptionKey', stored);
      }
