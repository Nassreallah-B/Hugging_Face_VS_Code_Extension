...
async function loadApiTokenCache() {
  try {
    if (_extensionContext) {
      const stored = await _extensionContext.secrets.get('hfaicode.apiToken');
...
async function saveApiTokenSecure(token) {
  ...
  if (_extensionContext) {
    await _extensionContext.secrets.store('hfaicode.apiToken', token || '');
  }
}
...
function getWorkspaceStorageRoot(context) {
  ...
  if (folder) return path.join(folder.uri.fsPath, '.hfaicode', 'storage');
