...
async function runAsyncTests() {
  try {
    ...
    results.fail++;
    results.errors.push('Swarm Execute: ' + e.message);
    console.log(`  ❌ Swarm Execute crashed: ${e.message}`);
  }
  ...
  try {
    ...
    results.fail++;
    results.errors.push('CVE Scanner: ' + e.message);
    console.log(`  ❌ CVE Scanner crashed: ${e.message}`);
  }
  ...
  try {
    ...
    process.env.HFAICODE_ENCRYPTION_KEY = require('crypto').randomBytes(32).toString('hex');
    ...
    results.fail++;
    results.errors.push('Encryption: ' + e.message);
    console.log(`  ❌ Encryption crashed: ${e.message}`);
  }
  ...
  try {
    ...
    results.fail++;
    results.errors.push('Hooks & Workers: ' + e.message);
    console.log(`  ❌ Hooks & Workers crashed: ${e.message}`);
  }
