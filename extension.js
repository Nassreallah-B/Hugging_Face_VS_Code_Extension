...
const {
  ...
  cloneMessages,
  cloneLogs,
  mergeLogEntries: sharedMergeLogEntries
} = require('./lib/shared');
...
const DEFAULT_SANDBOX_IMAGE = 'hf-ai-code-sandbox:latest';
...
const LOCAL_AGENT_TOOL_SPECS_BASE = [...AGENT_TOOL_SPECS, ...ADVANCED_AGENT_TOOL_SPECS, ...LOCAL_ONLY_AGENT_TOOL_SPECS];
function getLocalAgentToolSpecs() {
  const specs = [...LOCAL_AGENT_TOOL_SPECS_BASE];
  if (pluginManager) {
    const pluginTools = pluginManager.getToolSpecs();
    for (const pt of pluginTools) {
      if (!specs.some(t => t.name === pt.name)) specs.push(pt);
    }
  }
  return specs;
}
...
function mergeLogEntries(existingLogs, incomingLogs) {
  return sharedMergeLogEntries(existingLogs, incomingLogs, MAX_TASK_LOG_ENTRIES);
}
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    try {
      ...
      if (attempt < maxRetries) {
        ...
        console.warn(`[hf-ai-code] Embedding API attempt ${attempt + 1}/${maxRetries} failed, retrying in ${totalDelayMs}ms: ${error.message}`);
...
function readWorkspaceAgentsInstructions() {
  ...
  try {
    ...
    console.error(`[hf-ai-code] Failed to read AGENTS.md: ${err.message}`);
    return '';
  }
}
...
function collectWorkspaceEntries(basePath, options = {}) {
  ...
  const visit = (currentPath, currentDepth) => {
    ...
    try {
      ...
      console.error(`[hf-ai-code] Failed to read directory ${currentPath}: ${err.message}`);
      return;
    }
...
function searchWorkspaceText(pattern, options = {}) {
  ...
  for (const entry of files) {
    ...
    try {
      ...
      console.error(`[hf-ai-code] Error reading file during workspace search: ${err.message}`);
    }
  }
...
async function executeAgentToolCall(toolCall, execContext = {}) {
  ...
  switch (name) {
    ...
    default:
      // Route to plugin system if available
      if (pluginManager) {
...
class PersistentState {
  ...
  deleteChat(chatId) {
    ...
    try { fs.rmSync(this.getChatMessagePath(chatId), { force: true }); } catch (err) { console.error(`[hf-ai-code] Failed to delete chat messages for ${chatId}: ${err.message}`); }
    try { fs.rmSync(this.getChatSummaryPath(chatId), { force: true }); } catch (err) { console.error(`[hf-ai-code] Failed to delete chat summary for ${chatId}: ${err.message}`); }
...
class RagIndexManager {
  ...
  startAutoRefresh() {
    ...
    this.autoRefreshTimer = setTimeout(() => {
      ...
      this.performAutoRefresh().catch(error => {
        console.error(`[hf-ai-code] RAG auto-refresh failed: ${error.message}`);
      });
    }, intervalMs);
    console.log(`[hf-ai-code] RAG auto-refresh scheduled every ${intervalMinutes} minute(s).`);
  }
  ...
  async performAutoRefresh() {
    ...
    if (now - this.lastManualRebuildAt < recentThresholdMs) {
      console.log('[hf-ai-code] RAG auto-refresh skipped, index was recently rebuilt.');
      this.startAutoRefresh(); // Reschedule
      return;
    }
    console.log('[hf-ai-code] RAG auto-refresh triggered.');
  ...
  async buildChunksForFile(fsPath, oldByKey) {
    try {
      ...
      console.error(`[hf-ai-code] Error chunking file for RAG: ${err.message}`);
      return [];
    }
  }
  ...
  async getQueryEmbedding(queryText) {
    try {
      ...
      console.error(`[hf-ai-code] RAG query embedding failed: ${err.message}`);
      return null;
    }
  }
...
class HFAIRuntime {
  ...
  async refreshRuntimeContexts() {
    await vscode.commands.executeCommand('setContext', 'hfaicode.viewingDiff', this.store.getPendingPatches().length > 0);
    await vscode.commands.executeCommand('setContext', 'hfaicode.pendingQuestion', Boolean(this.features.getSnapshot().pendingQuestions.length));
  }
  ...
  async maybeAutoStartDocker(options = {}) {
    ...
    if (!dockerDesktopPath) {
      ...
      this._lastDockerAutoStartResult = result;
      console.warn(`[hf-ai-code] Docker auto-start skipped (${reason}): ${result.detail}`);
      return result;
    }
    ...
    try {
      ...
      this._lastDockerAutoStartResult = result;
      console.log(`[hf-ai-code] Docker auto-start (${reason}): ${result.detail}`);
      return result;
    } catch (error) {
      ...
      this._lastDockerAutoStartAt = now;
      this._lastDockerAutoStartResult = result;
      console.warn(`[hf-ai-code] Docker auto-start failed (${reason}): ${message}`);
      return result;
    }
  }
  ...
  async reviewPatch(patchId, filePath = '') {
    ...
    await vscode.commands.executeCommand('vscode.diff', leftUri, rightUri, `HF AI Patch Review: ${target.path}`);
    return patch;
  }
  ...
  async maybeCompactChat(chatId) {
    ...
    try {
      ...
      try {
        ...
        console.error(`[hf-ai-code] LLM-based chat summary failed: ${err.message}`);
        payload = null;
      }
  ...
  async buildWorkspaceSnapshot(rootPath) {
    ...
    while (queue.length && files.length < maxFiles && !truncated) {
      ...
      try {
        ...
        console.error(`[hf-ai-code] Failed to read directory for cloud snapshot: ${err.message}`);
        continue;
      }
      ...
      for (const entry of dirEntries) {
        ...
        try {
          ...
          console.error(`[hf-ai-code] Failed to stat file for cloud snapshot: ${err.message}`);
          continue;
        }
        ...
        try {
          ...
          console.error(`[hf-ai-code] Failed to read file for cloud snapshot: ${err.message}`);
          continue;
        }
    ...
    if (truncated) {
      ...
      console.warn(`[hf-ai-code] Cloud snapshot truncated: ${truncatedReason}`);
    }
    ...
    console.log(`[hf-ai-code] Cloud snapshot built: ${files.length} file(s), ${(totalBytes / 1024).toFixed(1)}KB total` +
  ...
  async createRemoteTask(localTask) {
    ...
    if (snapshot.truncated && snapshot.warnings && snapshot.warnings.length > 0) {
      vscode.window.showWarningMessage(
        `Cloud workspace snapshot was truncated: ${snapshot.warnings[0]}. Consider adjusting hfaicode.cloud.maxSnapshotFiles or hfaicode.cloud.maxSnapshotTotalBytes.`
      );
    }
    ...
    try {
      ...
      useCompressed = true;
      console.log(`[hf-ai-code] Cloud snapshot compressed: ${compressedData.originalBytes}B -> ${compressedData.compressedBytes}B (${compressedData.compressionRatio}x)`);
    } catch (err) {
      console.warn(`[hf-ai-code] Cloud snapshot compression failed, sending uncompressed: ${err.message}`);
    }
    ...
    try {
      for (const task of candidates) {
        try {
          ...
          console.error(`[hf-ai-code] Failed to sync task ${task.id}: ${err.message}`);
        }
      }
    } finally {
      this.syncInFlight = false;
...
function formatApiError(statusCode, errorData, modelId) {
  ...
  if (message.includes('api-inference.huggingface.co')) {
    return 'A legacy Hugging Face endpoint was called. Reload VS Code so the Router-based extension code is used.';
  }

  if (statusCode === 401 || statusCode === 403) {
    return 'Authentication failed. Use a Hugging Face token with "Make calls to Inference Providers" permission.';
  }

  if (statusCode === 404) {
    return `Model "${modelId}" was not found or is not available through Hugging Face Inference Providers.`;
  }
...
async function apiRequest(endpoint, body, onChunk, requestOptions = {}) {
  ...
  if (!token) {
    throw new Error('Hugging Face API Token not configured. Please add it in settings.');
  }
    ...
    const req = https.request(options, (res) => {
      ...
      const processSseEvent = (eventBlock) => {
        ...
        try {
          ...
          console.error(`[hf-ai-code] Failed to parse SSE chunk: ${err.message}`);
        }
      };
...
async function checkConnection() {
  const token = getApiToken();
  if (!token) {
    setConnectionState(false, 'No Hugging Face API token configured.');
    return false;
  }
        ...
        res.on('end', () => {
          if (res.statusCode === 200) {
            ...
            if (!modelAvailable) {
              setConnectionState(
                false,
                `Token valide, mais le modele "${selectedModel}" n'est pas disponible via Hugging Face Inference Providers chat. Essaye "${FALLBACK_MODEL_ID}".`
              );
              resolve(false);
              return;
            }
...
function updateStatus() {
  ...
  if (isConnected) {
    ...
    statusBarItem.tooltip = `Hugging Face Connected — Model: ${modelId}\nClick to change model`;
...
class HFInlineCompletionProvider {
  async provideInlineCompletionItems(document, position, context, token) {
    ...
    try {
      ...
      console.error(`[hf-ai-code] Inline completion failed: ${err.message}`);
      return [];
    }
  }
}
...
class HFChatViewProvider {
      ...
      try {
        ...
        switch (msg.type) {
          ...
          case 'saveInstructions': {
            ...
            if (scope === 'global') {
              await vscode.workspace.getConfiguration('hfaicode').update('instructions.global', text, vscode.ConfigurationTarget.Global);
            } else if (scope === 'workspace') {
              if (!getWorkspaceFolder()) {
                vscode.window.showErrorMessage('Workspace instructions require an open workspace.');
                break;
              }
              await vscode.workspace.getConfiguration('hfaicode').update('instructions.workspace', text, vscode.ConfigurationTarget.Workspace);
            } else if (scope === 'chat') {
          ...
          case 'saveModel': {
            ...
            await vscode.workspace.getConfiguration('hfaicode').update('modelId', modelId, vscode.ConfigurationTarget.Global);
            await this._doCheckConnection();
            break;
          }
        }
  ...
  _logAuditDecision(chatId, meta) {
    if (!meta) return;
    console.info('[hf-ai-code] intent=' + (meta.intent || 'general') + ' strict=' + (meta.strictAuditMode ? 'on' : 'off') + ' validation=' + (meta.postValidationStatus || 'pending') + ' chat=' + (chatId || 'none'));
  }
  ...
  async _doCheckConnection() {
    ...
    await this.syncState();
    if (connected) {
      vscode.window.showInformationMessage('Successfully connected to Hugging Face Router!');
    } else {
  ...
  async _selectModel() {
    ...
    const pick = await vscode.window.showQuickPick(
      [
        { label: 'Enter custom Model ID...', detail: 'Input any model from the Hugging Face Hub' },
        ...popularModels.map(m => ({ label: m }))
      ],
      { placeHolder: 'Select or enter a model ID', title: 'HF AI: Select Model' }
    );
    ...
    if (pick) {
      ...
      if (pick.label === 'Enter custom Model ID...') {
        finalModel = await vscode.window.showInputBox({
          prompt: 'Enter a Hugging Face chat model ID available through Inference Providers',
      ...
      if (finalModel) {
        ...
        await vscode.workspace.getConfiguration('hfaicode').update('modelId', finalModel, vscode.ConfigurationTarget.Global);
        this._post({ type: 'checking' });
        await this._doCheckConnection();
      }
    }
  ...
  async sendPromptFromCommand(text) {
    await vscode.commands.executeCommand('hfaicode.chatView.focus');
  ...
  _getHtml(webview) {
    ...
    try {
      ...
      // Replace brand names
      html = html.replace(/LocalAI/g, 'HF AI');
      return html;
    } catch (_) {
...
function getFallbackHtml(nonce, cspSource) {
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'nonce-${nonce}'; style-src ${cspSource} 'unsafe-inline';"><title>HF AI</title></head><body><p>Loading...</p></body></html>`;
}
...
async function activate(context) {
  ...
  statusBarItem.command = 'hfaicode.selectModel';
  statusBarItem.text = '$(sync~spin) HF AI: Connecting...';
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider('hfaicode.chatView', chatProvider, {
  ...
  // Commands
  context.subscriptions.push(
    vscode.commands.registerCommand('hfaicode.openChat', () => vscode.commands.executeCommand('hfaicode.chatView.focus')),
    vscode.commands.registerCommand('hfaicode.newChat', async () => {
      await appRuntime.initialize();
      appRuntime.store.createChat('New Chat');
      if (chatProvider) await chatProvider.syncState();
      await vscode.commands.executeCommand('hfaicode.chatView.focus');
    }),
    vscode.commands.registerCommand('hfaicode.explainCode', () => runCodeAction('explain')),
    vscode.commands.registerCommand('hfaicode.fixCode', () => runCodeAction('fix')),
    vscode.commands.registerCommand('hfaicode.refactorCode', () => runCodeAction('refactor')),
    vscode.commands.registerCommand('hfaicode.optimizeCode', () => runCodeAction('optimize')),
    vscode.commands.registerCommand('hfaicode.generateTests', () => runCodeAction('tests')),
    vscode.commands.registerCommand('hfaicode.addComments', () => runCodeAction('comments')),
    vscode.commands.registerCommand('hfaicode.generateCode', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) return;
    ...
    vscode.commands.registerCommand('hfaicode.askAboutSelection', async () => {
    ...
    vscode.commands.registerCommand('hfaicode.selectModel', async () => {
      if (chatProvider) await chatProvider._selectModel();
    }),
    vscode.commands.registerCommand('hfaicode.checkConnection', async () => {
    ...
    vscode.commands.registerCommand('hfaicode.acceptDiff', async () => {
      ...
      if (!patch) {
        vscode.window.showInformationMessage('No pending HF AI patch to apply.');
        return;
      }
    ...
    vscode.commands.registerCommand('hfaicode.rejectDiff', async () => {
      await appRuntime.initialize();
      const patch = appRuntime.store.getPendingPatches()[0];
      if (!patch) {
        vscode.window.showInformationMessage('No pending HF AI patch to reject.');
        return;
      }
    ...
    vscode.commands.registerCommand('hfaicode.reviewDiff', async () => {
      await appRuntime.initialize();
      const patch = appRuntime.store.getPendingPatches()[0];
      if (!patch) {
        vscode.window.showInformationMessage('No pending HF AI patch to review.');
        return;
      }
  ...
  setTimeout(async () => {
    try {
      ...
      // MutationGuard — fail-closed write/shell/delete validation with approval callback
  ...
  const configListener = vscode.workspace.onDidChangeConfiguration(async (event) => {
    if (event.affectsConfiguration('hfaicode.memory.scope')) {
