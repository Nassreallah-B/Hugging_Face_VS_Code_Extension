...
const DEFAULT_MODEL_ID = process.env.HF_MODEL_ID || 'Qwen/Qwen2.5-Coder-32B-Instruct:fastest';
...
const DEFAULT_SANDBOX_IMAGE = process.env.CLOUD_EXECUTOR_SANDBOX_IMAGE || 'hf-ai-code-sandbox:latest';
...
function requestHfChat(messages, task, tokenOverride) {
  const token = tokenOverride || process.env.HF_API_TOKEN || '';
  if (!token) throw new Error('No Hugging Face API token available on the executor. Set HF_API_TOKEN on the server or forward a token from the extension.');
  const body = JSON.stringify({
    model: task.modelId || DEFAULT_MODEL_ID,
    messages,
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      res.on('end', () => {
        if (res.statusCode !== 200) {
          reject(new Error(`HF Router error (${res.statusCode}): ${extractErrorMessage(raw) || raw}`));
          return;
        }
    ...
    req.on('error', reject);
    req.on('timeout', () => { req.destroy(); reject(new Error('HF Router request timeout')); });
    req.write(body);
    req.end();
  });
}
...
class CloudTaskStore {
  ...
  createTask(input) {
    ...
    const task = {
      ...
      tokenSource: input.tokenSource || (process.env.HF_API_TOKEN ? 'server' : 'forwarded')
    };
    this.saveTask(task);
    return task;
  }
...
class CloudTaskManager {
  ...
  getSandboxManager(config) {
    ...
    if (!this.sandboxManagers.has(key)) {
      this.sandboxManagers.set(key, new DockerSandboxManager({
        ...
        containerNamePrefix: 'hfai-cloud-sbx'
      }));
    }
  ...
  async createTask(input) {
    const serverToken = process.env.HF_API_TOKEN || '';
    const forwardedToken = String(input.hfApiToken || '').trim();
    if (!serverToken && !forwardedToken) {
      throw new Error('No Hugging Face token is available for the executor. Set HF_API_TOKEN on the server or enable token forwarding in the extension.');
    }
    ...
    const task = this.store.createTask({
      ...
      tokenSource: serverToken ? 'server' : 'forwarded'
    });

    if (forwardedToken) this.forwardedTokens.set(task.id, forwardedToken);
  ...
  async startTask(taskId) {
    ...
    try {
      ...
      const tokenOverride = this.forwardedTokens.get(taskId) || '';
      ...
      for (; ;) {
        ...
        if (checkpoint && checkpoint.phase === 'executing_tools' && Array.isArray(checkpoint.pendingToolCalls) && checkpoint.pendingToolCalls.length) {
          ...
          const result = await requestHfChat(conversation, currentTask, tokenOverride);
  ...
  try {
    ...
    if (req.method === 'GET' && requestUrl.pathname === '/health') {
      ...
      sendJson(res, 200, {
        ...
        dataRoot: DATA_ROOT,
        serverTokenConfigured: Boolean(process.env.HF_API_TOKEN),
        sandbox: {
...
async function main() {
  ...
  server.listen(PORT, '0.0.0.0', () => {
    console.log(`HF AI cloud executor listening on http://127.0.0.1:${PORT}`);
  });
}
