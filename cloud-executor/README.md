...
- `server.js` : serveur HTTP, store de tâches, orchestration agent distante, intégration sandbox et appels Hugging Face
...
- Node.js
- Docker actif
- image `hf-ai-code-sandbox:latest` disponible ou autobuild activé
- `HF_API_TOKEN` configuré côté serveur ou forwarding volontaire côté client
